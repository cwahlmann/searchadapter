...
Diese Methode muss dann zunächst den korrekten *Index* auf die Input-Daten ermitteln,
...
[source, Java]
.Die vollständige Haupt-Suchmethode
----
...
Für die Caches nutze ich `Caffeine`.
...
abstract PagedSearchResult<T> find(PagedSearch<U> pagedSearch); <3>
----
<1> Der Cache für Input-Pages, abhängig von der aktuellen Suche.
<2> Der Cache für Indexes, abhängig von der aktuellen Suche und dem aktuellen, zusätzlichen Filter.
<3> Diese Methode muss für eine Suche mit der konkreten Input-API implementiert werden.

Den gesamten Quellcode für das `SearchWithFilterAdapter` gibt es
