...
Gerade die zweite Anforderung kann ein Cache normalerweise nicht leisten, da Caches als Map organisiert sind.
Der Key liegt also nicht als sortierte Liste vor, auf der man den einen Wert finden könnte, der maximal ist, aber
kleiner als ein bestimmter Wert. Der Cache muss deswegen beginnend von der gesuchten Seite abwärts nach einem
bereits bekannten Index durchsucht werden.
